...
from ...agent.world_model import is_7tan_model, ensure_7tan_model_config, get_tan_model
...
def _7tan_model_reply(used_model: str, user_message: str, session_id: str = "") -> str:
    """调用本地 7Tan 模型引擎，返回预测报告 / 言语自述文本

    session_id: 用于读取该会话的历史（读上下文能力）；为空则退化为无上下文。
    """
    ensure_7tan_model_config()
    logger.info(f"🌍 7Tan模型（本地引擎）处理消息: {user_message[:50]}")
    history: list = []
    if session_id:
        try:
            ...
            # 去掉最后一条（当前消息，已作为 user_message 传入，避免重复）
            if history and history[-1].get("role") == "user" and history[-1].get("content", "").strip() == user_message.strip():
                history = history[:-1]
        except Exception as e:
            logger.warning(f"读取会话历史失败（退化为无上下文）: {e}")
    try:
        return get_tan_model().respond(user_message, history=history)
    except Exception as e:
        logger.error(f"7Tan 模型响应失败: {e}")
        return f"❌ 7Tan 模型引擎异常: {e}"
...
def _interrupt_prev_speech(reason: str = "") -> None:
    """新用户消息到达 → 立即打断上一次回复的朗读。

    实时控制台的 💬 Agent 朗读会同步停止（清空待读缓冲 + 通知播放线程停止当前音频）；
    本轮新回复的 💬 Agent 内容随后会重新触发朗读，不会与旧语音叠加。

    ⚠️ 必须保持「非阻塞」：本函数在 asyncio 事件循环里同步执行，
    一旦内部出现阻塞调用（历史事故：winsound 的 SND_PURGE 会阻塞到音频播完），
    整个 Web 服务都会卡住 → 前端 12 秒超时 → 误弹「网页版服务未运行」。
    现由 speak_bridge.interrupt_speaking() 保证快速返回（内含耗时告警兜底）。
    """
    try:
        from ...agent.speak_bridge import interrupt_speaking
        interrupt_speaking(reason)
    except Exception as e:
        logger.debug(f"[TTS] 打断上一次朗读失败: {e}")


@router.post("/send")
async def send_message(req: ChatRequest):
    """发送对话消息"""
    _interrupt_prev_speech("api_chat_send")
    ...
    if is_7tan_model(used_model):
        reply = _7tan_model_reply(used_model, user_message, session_id)
        save_message(session_id, "assistant", reply, model=used_model)
        return ChatResponse(session_id=session_id, success=True, result=reply,
                            cost=0, iterations=0, tokens=0, model=used_model)
...
async def send_message_stream(req: ChatRequest, request: Request):
    ...
    _interrupt_prev_speech("api_chat_send_stream")
    session_id = req.session_id or create_session()
    used_model = _resolve_model_name(req.model)
    # 组装用户消息（若带图片：先视觉分析，描述并入消息）
    user_message = _compose_message(req)
    ...
    if is_7tan_model(used_model):
        ...
        async def _wm_stream():
            _nl = chr(10)
            yield f"data: {json.dumps({'type': 'text_chunk', 'content': reply}, ensure_ascii=False)}{_nl}{_nl}"
            save_message(session_id, "assistant", reply, model=used_model)
            yield f"data: {json.dumps({'type': 'done', 'result': reply, 'model': used_model}, ensure_ascii=False)}{_nl}{_nl}"
            yield f"data: {json.dumps({'type': 'session_id', 'session_id': session_id})}{_nl}{_nl}"
