...
function refreshCurrent() {
  const fns = {
    ...
    adapt: loadAdaptView, mind: loadMind,
  };
...
/* ---------- 意识模型（7Tan 仿生器官框架实时状态） ---------- */
let mindTimer = null;
function stopMindPolling() {
  if (mindTimer) { clearInterval(mindTimer); mindTimer = null; }
}
function startMindPolling() {
  if (mindTimer) return;
  mindTimer = setInterval(() => { if (currentView === "mind") loadMind(); }, 3000);
}
$("#mindRefresh") && $("#mindRefresh").addEventListener("click", () => loadMind());
$("#mindAuto") && $("#mindAuto").addEventListener("change", (e) => {
  if (e.target.checked) { loadMind(); } else { stopMindPolling(); }
});
async function loadMind() {
  try {
    const d = await API.get("/api/mind/status");
    if (!d || d.ok === false) {
      $("#mindStatus").textContent = "⚠️ " + ((d && d.error) || "器官框架未挂载");
      stopMindPolling();
      return;
    }
    renderMind(d);
    $("#mindStatus").textContent = "🟢 已连接 · 更新于 " + new Date().toLocaleTimeString();
    if ($("#mindAuto") && $("#mindAuto").checked) startMindPolling();
    else stopMindPolling();
  } catch (e) {
    $("#mindStatus").textContent = "❌ 加载失败: " + e.message;
    stopMindPolling();
  }
}
function _mindCard(cls, lbl, num) {
  return `<div class="card ${cls}"><div class="lbl">${lbl}</div><div class="num">${num}</div></div>`;
}
function renderMind(d) {
  const hb = d.heartbeat || {};
  $("#mindHeartbeat").innerHTML = [
    _mindCard("blue", "🫀 心跳 tick", hb.tick_counter),
    _mindCard("violet", "⏱ 主观时钟", hb.subjective_clock),
    _mindCard("yellow", "🌀 时间膨胀", hb.time_dilation),
    _mindCard(hb.alive ? "green" : "", "💓 心跳线程", hb.alive ? "存活" : "停止"),
  ].join("");

  $("#mindOrganCount").textContent = `（${d.enabled_count}/${d.organ_count} 启用）`;
  $("#mindOrgans").innerHTML = (d.organs || []).map((o) => `
    <div class="organ-card ${o.enabled ? "" : "off"}">
      <div class="organ-head"><b>${escapeHtml(o.code)}</b><span class="tag ${o.enabled ? "published" : "failed"}">${o.enabled ? "启用" : "停用"}</span></div>
      <div class="organ-name">${escapeHtml(o.name)}</div>
      <div class="organ-bio muted">${escapeHtml(o.bio)}</div>
    </div>`).join("") || "<div class='muted'>无器官</div>";

  const sm = d.self_model || {};
  const cap = sm.capability || []; const lim = sm.limitations || []; const flaws = sm.mind_flaws || [];
  const hs = sm.history_summary || {};
  const conf = typeof sm.confidence === "number" ? (sm.confidence * 100).toFixed(1) + "%" : "-";
  let smHtml = `<div class="mind-kv">置信度 confidence：<b>${conf}</b></div>`;
  smHtml += `<div class="mind-kv">能力条目 capability：<b>${cap.length}</b></div>`;
  if (cap.length) smHtml += `<div class="muted" style="margin:2px 0 8px">${cap.slice(0, 6).map((c) => escapeHtml(c && (c.key || c.goal_tag || ""))).filter(Boolean).join("、") || "（结构化条目）"}</div>`;
  smHtml += `<div class="mind-kv">已知局限 limitations：<b>${lim.length}</b></div>`;
  smHtml += `<div class="mind-kv">思维缺陷 mind_flaws：<b>${flaws.length}</b> ${flaws.length ? "⚠️" : "✅"}</div>`;
  smHtml += `<div class="mind-kv">目标统计：成功 <b>${hs.total_success || 0}</b> / 失败 <b>${hs.total_fail || 0}</b> / 总 <b>${hs.total_goal || 0}</b></div>`;
  $("#mindSelfModel").innerHTML = smHtml;

  const wm = d.working_memory || {};
  let wHtml = `<div class="mind-kv">活跃目标 active_goals：<b>${(wm.active_goals || []).length}</b></div>`;
  if ((wm.active_goals || []).length) wHtml += `<div class="muted" style="margin:2px 0 8px">${(wm.active_goals || []).map(escapeHtml).join("、")}</div>`;
  wHtml += `<div class="mind-kv">上下文栈 context_stack：<b>${wm.context_stack_len}</b></div>`;
  $("#mindWorkingMemory").innerHTML = wHtml;

  const mc = d.meta_cognition || {};
  $("#mindMetaCognition").innerHTML = `<div class="mind-kv">假设接纳 <b>${mc.hypo_accepted || 0}</b> / 证伪 <b>${mc.hypo_rejected || 0}</b></div><div class="mind-kv">预测误差样本 <b>${mc.pe_sample_count || 0}</b> · 近20均值 <b>${mc.pe_mean_recent == null ? "-" : mc.pe_mean_recent}</b></div>`;

  $("#mindMemoryCount").textContent = `（共 ${d.memory_total} 条，显示最近 ${(d.memory || []).length} 条）`;
  $("#mindMemory").innerHTML = (d.memory || []).map((m) => `
    <div class="mem-item">
      <div class="mem-thought">${escapeHtml(m.thought)}</div>
      <div class="mem-meta muted">效价 ${m.valence == null ? "-" : m.valence} · 权重 ${m.weight_eff == null ? "-" : m.weight_eff} · 成败 ${m.success == null ? "-" : m.success} · PE ${m.pe == null ? "-" : m.pe}${m.goal_tag ? " · 目标 " + escapeHtml(m.goal_tag) : ""}${m.subjective_time != null ? " · 主观时 " + m.subjective_time : ""}</div>
    </div>`).join("") || "<div class='muted'>暂无长期记忆</div>";

  $("#mindHypoCount").textContent = `（共 ${d.hypothesis_count} 条）`;
  $("#mindHypotheses").innerHTML = (d.hypotheses || []).map((h) => `
    <div class="mem-item"><div class="mem-thought">${escapeHtml(h.hypo || h.key || "")}</div><div class="mem-meta muted">tests=${h.tests} · score=${h.score}</div></div>`).join("") || "<div class='muted'>暂无假设</div>";

  const os = d.origin_stats || {};
  const osTotal = (os.internal || 0) + (os.external || 0);
  $("#mindOrigin").innerHTML = `<div class="mind-kv">内部信号 internal：<b>${os.internal || 0}</b></div><div class="mind-kv">外部输入 external：<b>${os.external || 0}</b></div><div class="muted" style="margin-top:6px">我-非我边界：区分「自身思维」与「外界输入」的证据来源（共 ${osTotal}）</div>`;

  $("#mindEvents").textContent = (d.recent_events || []).map((e) => `[${e.ts || ""}] #${e.tick || 0} ${e.type || ""}${e.origin ? " ←" + e.origin : ""}`).join("\n") || "暂无事件";

  renderMindGrowth(d.growth);
  renderMindLearning(d.learning);
}

function renderMindGrowth(g) {
  const el = $("#mindGrowth");
  if (!el) return;
  g = g || {};
  if (g.ok !== true) {
    el.innerHTML = `<div class="muted">${g.error ? escapeHtml(g.error) : "暂无心智成长数据"}</div>`;
    return;
  }
  let html = `<div class="mind-kv">主观心智年龄：<b>${g.subj_now == null ? "-" : g.subj_now}</b></div>`;
  html += `<div class="mind-kv">记忆库 <b>${g.memory_count || 0}</b> 条 · 关键经历 <b>${g.key_memory_count || 0}</b> 条</div>`;
  html += `<div class="mind-kv">心智阶段 <b>${g.stage_count || 0}</b> 个 · 可溯源 <b>${g.traceable_count || 0}</b> 条</div>`;
  if ((g.stages || []).length) {
    html += `<div class="muted" style="margin:2px 0 8px">${(g.stages || []).map(s => `${s.start_subj}→${s.end_subj}(${s.count})`).join(" · ")}</div>`;
  }
  if ((g.conflicts || []).length) {
    html += `<div class="mind-kv">认知矛盾 <b>${g.conflicts.length}</b> ⚠️</div>`;
    html += `<div class="muted" style="margin:2px 0 8px">${(g.conflicts || []).map(c => escapeHtml(c.detail || c.type || "")).join("<br>")}</div>`;
  } else {
    html += `<div class="mind-kv">认知矛盾 <b>0</b> ✅</div>`;
  }
  if ((g.high_freq_motives || []).length) {
    html += `<div class="mind-kv">高频动机：<b>${(g.high_freq_motives || []).map(m => escapeHtml(m[0]) + "×" + m[1]).join("、")}</b></div>`;
  }
  const pe = g.pe_classification || {};
  html += `<div class="mind-kv">预测误差：正 <b>${pe.pos || 0}</b> / 负 <b>${pe.neg || 0}</b> / 零 <b>${pe.zero || 0}</b></div>`;
  html += `<div class="mind-kv">记忆留存率：<b>${g.memory_retention == null ? "-" : (g.memory_retention * 100).toFixed(1) + "%"}</b></div>`;
  el.innerHTML = html;
}

function renderMindLearning(ln) {
  if (!ln) return;
  const l1 = ln.l1 || {}, l2 = ln.l2 || {}, m1 = ln.m1 || {};
  const hs = l1.hypotheses || {}, st = hs.stats || {};
  let summary = `🧪 假设：共 <b>${st.total || 0}</b>（已验证 ${st.verified || 0} / 待定 ${st.suspended || 0} / 拒绝 ${st.rejected || 0}）`;
  summary += ` ｜ 💬 对话经验 <b>${l1.dialog_total || 0}</b> ｜ 📚 知识条目 <b>${l1.knowledge_total || 0}</b>`;
  const es = l2.status || {};
  if (es.enabled !== undefined) {
    summary += `<br>🔧 进化：${es.enabled ? "✅ 开启" : "⛔ 关闭"} ｜ 今日 ${es.today_attempts || 0}/${es.daily_quota || 0} ｜ 冷却 ${es.cooldown_left || 0}s`;
  }
  summary += `<br>📦 知识获取(M1)：${m1.mounted ? "✅ 已挂载" : "⛔ 未挂载"} ｜ 待投递 <b>${m1.package_pending || 0}</b> ｜ 已处理 <b>${m1.package_processed || 0}</b> ｜ 自学队列 <b>${m1.web_queue || 0}</b> ｜ 限速 <b>${m1.rate_limit || 0}</b>/tick`;
  const smEl = $("#mindLearnSummary"); if (smEl) smEl.innerHTML = summary;

  const dialogEl = $("#mindDialog"); if (dialogEl) dialogEl.innerHTML = (l1.dialog || []).map((x) => `
    <div class="mem-item"><div class="mem-thought">${escapeHtml(x.user_text || "")}</div><div class="mem-meta muted">→ ${escapeHtml(x.reply || "")}${x.action ? " · " + escapeHtml(x.action) : ""}</div></div>`).join("") || "<div class='muted'>暂无对话经验</div>";

  const knowEl = $("#mindKnowledge"); if (knowEl) knowEl.innerHTML = (l1.knowledge || []).map((k) => `
    <div class="mem-item"><div class="mem-thought">${escapeHtml(k.key || "")}</div><div class="mem-meta muted">${escapeHtml(k.content || "")}${k.source ? " · 来源 " + escapeHtml(k.source) : ""}</div></div>`).join("") || "<div class='muted'>暂无知识条目</div>";

  const hypoEl = $("#mindHypoDetail"); if (hypoEl) hypoEl.innerHTML = (hs.recent || []).map((h) => `
    <div class="mem-item"><div class="mem-thought">${escapeHtml(h.content || h.id || "")}</div><div class="mem-meta muted"><span class="tag ${h.status === "verified" ? "published" : (h.status === "rejected" ? "failed" : "")}">${escapeHtml(h.status || "?")}</span> 置信 ${h.confidence == null ? "-" : h.confidence} · 验证 ${h.success_count || 0}/${h.test_count || 0}</div></div>`).join("") || "<div class='muted'>暂无假设</div>";

  const evEl = $("#mindEvolve"); if (evEl) {
    if (l2.ready) {
      const cs = l2.constants || {};
      let h = `<div class="mind-kv">进化白名单常量：</div>`;
      h += `<div class="muted" style="margin:2px 0 8px;word-break:break-all">${Object.entries(cs).map(([k, v]) => `${k}=${v}`).join("、") || "（无）"}</div>`;
      if (es.last_result) h += `<div class="mind-kv">最近进化结果：<b>${escapeHtml(JSON.stringify(es.last_result).slice(0, 140))}</b></div>`;
      h += `<div class="mind-kv" style="margin-top:6px">最近进化日志：</div><pre class="mono" style="max-height:130px;overflow:auto;font-size:11px">${(l2.recent_log || []).map(escapeHtml).join("\n") || "（空）"}</pre>`;
      evEl.innerHTML = h;
    } else {
      evEl.innerHTML = "<div class='muted'>进化模块未初始化</div>";
    }
  }
}

$("#mindLearnWeb") && $("#mindLearnWeb").addEventListener("click", async () => {
  const url = ($("#mindLearnUrl").value || "").trim();
  if (!url) { toast("请输入网页 URL", true); return; }
  const btn = $("#mindLearnWeb"); btn.disabled = true; btn.textContent = "⏳ 抓取中…";
  try {
    const r = await API.post("/api/mind/knowledge-package", { url });
    toast(r.ok ? `✅ 已抓取，排队 ${r.queued} 个片段（经 tick 链路吸收）` : ("⚠️ " + (r.error || "失败")), !r.ok);
    if (r.ok) { $("#mindLearnUrl").value = ""; loadMind(); }
  } catch (e) { toast("抓取失败: " + e.message, true); }
  finally { btn.disabled = false; btn.textContent = "⬇️ 抓取自学"; }
});
$("#mindLearnImport") && $("#mindLearnImport").addEventListener("click", async () => {
  const text = ($("#mindLearnText").value || "").trim();
  if (!text) { toast("请粘贴知识内容", true); return; }
  const entries = text.split("\n").map((s) => s.trim()).filter(Boolean)
    .map((s, i) => ({ item_id: "manual-" + Date.now() + "-" + i, content: s }));
  const btn = $("#mindLearnImport"); btn.disabled = true; btn.textContent = "⏳ 导入中…";
  try {
    const r = await API.post("/api/mind/knowledge-package", { entries });
    if (r.ok) {
      const dup = r.duplicates ? `，${r.duplicates} 条已存在跳过` : "";
      toast(`✅ 已导入 ${r.imported} 条${dup}｜队列 ${r.pending} 条待学习`, false);
      $("#mindLearnText").value = ""; loadMind();
    } else { toast("⚠️ " + (r.error || "失败"), true); }
  } catch (e) { toast("导入失败: " + e.message, true); }
  finally { btn.disabled = false; btn.textContent = "📥 导入知识包"; }
});

/* 知识包文件导入：抽公共函数，供「选择文件」「打开文件夹」「拖拽」三个入口复用 */
async function handleMindFiles(files, btn) {
  const list = Array.from(files || []);
  if (!list.length) return;
  const orig = btn ? btn.textContent : "";
  const MAX_FILE = 2 * 1024 * 1024;
  const MAX_TOTAL = 5000;
  const extRe = /\.(txt|md|json|csv|log|text|docx)$/i;
  let entries = [], skipped = 0, docxFiles = [];
  for (const f of list) {
    if (!extRe.test(f.name)) { skipped++; continue; }
    if (f.size > MAX_FILE) { skipped++; toast(`跳过 ${f.name}（>2MB）`, true); continue; }
    const rel = f.webkitRelativePath || f.name;
    if (/\.docx$/i.test(f.name)) { docxFiles.push({ f, rel }); continue; }
    let text = "";
    try { text = await f.text(); } catch (e) { skipped++; continue; }
    text.split(/\r?\n/).map((s) => s.trim()).filter(Boolean)
      .forEach((content, i) => entries.push({ item_id: "file:" + rel + ":" + i, content }));
  }
  if (!entries.length && !docxFiles.length) { toast("没有可导入的文本内容", true); return; }
  if (entries.length > MAX_TOTAL) entries = entries.slice(0, MAX_TOTAL);
  if (btn) { btn.disabled = true; btn.textContent = "⏳ 导入中…"; }
  let total = 0, dupTotal = 0, pendingLeft = 0;
  try {
    if (entries.length) {
      const r = await API.post("/api/mind/knowledge-package", { entries });
      if (r.ok) { total += r.imported || 0; dupTotal += r.duplicates || 0; pendingLeft = r.pending || 0; }
      else toast("⚠️ " + (r.error || "文本导入失败"), true);
    }
    for (const { f, rel } of docxFiles) {
      const b64 = await new Promise((res, rej) => {
        const rd = new FileReader();
        rd.onload = () => { const s = String(rd.result || ""); res(s.includes(",") ? s.slice(s.indexOf(",") + 1) : s); };
        rd.onerror = rej; rd.readAsDataURL(f);
      });
      const r = await API.post("/api/mind/knowledge-docx", { base64: b64, filename: rel });
      if (r.ok) { total += r.imported || 0; dupTotal += r.duplicates || 0; pendingLeft = r.pending || 0; }
      else toast(`⚠️ ${f.name}: ${r.error || "导入失败"}`, true);
    }
    if (total > 0) {
      toast(`✅ 已导入 ${total} 条，队列 ${pendingLeft} 条待学习（心跳逐步吸收）`, false);
    } else if (dupTotal > 0) {
      toast(`⚠️ 内容已导入过（${dupTotal} 条已存在），无需重复导入；队列 ${pendingLeft} 条待学习`, true);
    } else {
      toast(`没有新增内容${skipped ? `（跳过 ${skipped} 个文件）` : ""}`, true);
    }
    loadMind();
  } catch (e) { toast("导入失败: " + e.message, true); }
...
$("#mindLearnPick") && $("#mindLearnPick").addEventListener("click", () => {
  const fi = $("#mindLearnFilePick"); if (fi) fi.click();
});
$("#mindLearnFolder") && $("#mindLearnFolder").addEventListener("click", () => {
  const fi = $("#mindLearnFile"); if (fi) fi.click();
});

$("#mindLearnFilePick") && $("#mindLearnFilePick").addEventListener("change", async function () {
  await handleMindFiles(this.files, $("#mindLearnPick"));
  this.value = "";
});
$("#mindLearnFile") && $("#mindLearnFile").addEventListener("change", async function () {
  await handleMindFiles(this.files, $("#mindLearnFolder"));
  this.value = "";
});

/* 拖拽导入：把文件拖到自我学习面板即导入 */
const _mindOps = $("#mindLearnOps");
if (_mindOps) {
  ["dragenter", "dragover"].forEach((ev) => _mindOps.addEventListener(ev, (e) => { e.preventDefault(); e.stopPropagation(); _mindOps.classList.add("mind-drop-active"); }));
  ["dragleave", "drop"].forEach((ev) => _mindOps.addEventListener(ev, (e) => { e.preventDefault(); e.stopPropagation(); _mindOps.classList.remove("mind-drop-active"); }));
  _mindOps.addEventListener("drop", (e) => { const fs = e.dataTransfer && e.dataTransfer.files; if (fs && fs.length) handleMindFiles(fs, null); });
}

/* ⚡ 立即学习全部：一次性消化知识包队列所有 pending（跳过 rate≤3 限速，用户手动触发） */
$("#mindLearnDrainAll") && $("#mindLearnDrainAll").addEventListener("click", async () => {
  const btn = $("#mindLearnDrainAll");
  btn.disabled = true; btn.textContent = "⏳ 学习中…";
  try {
    const r = await API.post("/api/mind/knowledge-drain-all", {});
    if (r.ok) {
      toast(`✅ 已立即学习 ${r.drained} 条，队列剩余 ${r.pending} 条`, false);
      loadMind();
    } else { toast("⚠️ " + (r.error || "失败"), true); }
  } catch (e) { toast("学习失败: " + e.message, true); }
  finally { btn.disabled = false; btn.textContent = "⚡ 立即学习全部"; }
});
...
function _onProviderChange(provider) {
  const presets = {
    ...
    sensetime: { base: "https://token.sensenova.cn/v1",            model: "sensenova-6.8-flash-lite" },
