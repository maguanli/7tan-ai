...
# 安全模块源码额外排除：token_store.py 含 token 加密 / 机器指纹方案的完整实现，
# 不随发布包分发（编译产物 src/security/token_store.pyd 已随包提供，不影响用户重建）。
_SECURITY_SRC_EXCLUDE = {
    "token_store.py",
}
...
def zip_dir(zf: zipfile.ZipFile, src_dir, arc_root: str,
            exclude_dirs=(), exclude_files=(), include_top_files=None) -> int:
    """递归打包目录，返回文件数

    exclude_dirs      : 目录名黑名单（路径任意段命中即跳过）
    exclude_files     : 文件名黑名单（支持 fnmatch 通配符）
    include_top_files : 顶层文件白名单（仅对 src 直属文件生效；None=不启用）
                        用于发布包只保留必需的顶层脚本（如 WEB 包的 tools/）
    """
    src = Path(src_dir)
    count = 0
    skipped = 0
    for p in sorted(src.rglob("*")):
        ...
        if any(part in exclude_dirs for part in rel.parts):
            skipped += 1
            continue
        if (include_top_files is not None and len(rel.parts) == 1
                and p.name not in include_top_files):
            skipped += 1
            continue
...
def add_source_bundle(zf: zipfile.ZipFile, project_root) -> int:
    ...
    for rel, arc in _SOURCE_DIRS:
        ...
        n = zip_dir(zf, src, arc,
                    ...
                    exclude_files=EXCLUDE_FILE_PATTERNS | _SECURITY_SRC_EXCLUDE)
        count += n
    return count
...
_WEB_ROOT_FILES = [
    ...
    # Windows 启动脚本不取源码根目录同名 bat（那是完整版 exe 专用，web 包无 exe/.venv），
    # 统一由下方 _WEB_WINDOWS_BAT 内置模板写入，避免同名条目重复。
    # ("启动网页版服务(Windows).bat",     "启动网页版服务(Windows).bat"),
]

# WEB 版 Windows 启动脚本（WEB 包专用：检测 Python → 装依赖 → 起服务 → 开浏览器）
_WEB_WINDOWS_BAT = (
    "@echo off\r\n"
    "chcp 65001 >nul\r\n"
    "setlocal\r\n"
    "cd /d \"%~dp0\"\r\n"
    "title 7Tan WEB 版启动器 (Windows)\r\n"
    "\r\n"
    "echo ==========================================\r\n"
    "echo   7Tan WEB 版 - Windows 启动\r\n"
    "echo   地址: http://127.0.0.1:9900/\r\n"
    "echo ==========================================\r\n"
    "echo.\r\n"
    "\r\n"
    ...
    "if errorlevel 1 (\r\n"
    "    echo [错误] 未检测到 Python。\r\n"
    "    echo        请先安装 Python 3.10+（安装时勾选 Add python.exe to PATH）:\r\n"
    ...
    "\r\n"
    "echo [1/3] 检查依赖...\r\n"
    "python -c \"import fastapi, uvicorn, yaml, sqlalchemy, requests, loguru\" >nul 2>&1\r\n"
    "if errorlevel 1 (\r\n"
    "    echo        正在安装依赖（首次运行，需联网，约1-2分钟）...\r\n"
    ...
    "\r\n"
    "echo [2/3] 正在启动服务...\r\n"
    "start \"\" http://127.0.0.1:9900/\r\n"
    "python web_main.py --host 127.0.0.1 --port 9900\r\n"
    "\r\n"
    "echo [3/3] 服务已退出。\r\n"
    "pause\r\n"
)
...
_WEB_DIRS = [
    ...
    ("tools",         "tools",         {"__pycache__", "ocr_work", "_secrets_backup",
                                        "gzh_publish"}),
]

# WEB 包 tools/ 顶层文件白名单
# ------------------------------------------------------------------
# tools/ 在源码仓库中同时充当"开发临时脚本目录"：183 个顶层文件里绝大多数
# 是一次性调试产物（_debug_ocr5.py / _scroll_hot_list.py / click_hot2.py ...），
# 以及目标平台无法运行的 Windows 工具：
#     cloudflared.exe           52.35 MB
#     python-3.12.5-amd64.exe   25.28 MB（+ install_python.bat）
# WEB 包面向 macOS / Linux / 鸿蒙，上述内容纯属占体积与噪音，
# 故改为"白名单制"：只保留经全量引用扫描确认被源码/插件引用的脚本。
# 4 个子目录（camera_vision / desktop_bot / viral_writer / security_src）
# 被插件与运行时引用，整目录保留。
# gzh_publish（微信公众号桌面自动化：pyautogui 坐标点击 + 剪贴板）
# 为 Windows 专属，非 Windows 系统无法运行，故从 WEB 包剔除；
# 对应插件 data/plugins/gzh_publish 会返回"缺少发布入口脚本"的友好提示。
_WEB_TOOLS_KEEP = {
    "_wait.py",
    "article_v3.py",
    "build.py",
...
_WEB_EXCLUDE_FILES = EXCLUDE_FILE_PATTERNS | _SECURITY_SRC_EXCLUDE | {
    "browser_bridge.py",
    "cookie_manager.py",
    "*.pyc",
}
        ...
        with zipfile.ZipFile(tmp_zip, "w", zipfile.ZIP_DEFLATED, compresslevel=ZIP_LEVEL) as zf:
            ...
            for rel, arc, extra_dirs in _WEB_DIRS:
                ...
                if not src.exists():
                    ...
                    continue
                # tools/ 走白名单：只保留运行时/维护确需的顶层脚本
                n = zip_dir(zf, src, arc,
                            exclude_dirs=EXCLUDE_DIRS | extra_dirs,
                            exclude_files=_WEB_EXCLUDE_FILES,
                            include_top_files=_WEB_TOOLS_KEEP if arc == "tools" else None)
                print(f"  [OK] {arc}/  {n} 文件")
                n_total += n
            # 3. 水印
            if build_id:
            ...
            # 4.1 Windows 启动脚本（始终用内置模板：源码根目录同名 bat 是
            #     完整版 exe 专用，web 包无 exe/.venv，必须用 web 适配模板）
            zf.writestr("启动网页版服务(Windows).bat", _WEB_WINDOWS_BAT)
            print(f"  [OK] 启动网页版服务(Windows).bat (web 内置模板)")
            # 5. 说明书
...
def main() -> int:
    ...
    try:
        with zipfile.ZipFile(tmp_zip, "w", zipfile.ZIP_DEFLATED, compresslevel=ZIP_LEVEL) as zf:
            ...
            # 2.1.1 exe 旁 data/logo（server.py 的 /logo 静态目录与图标资源）
            _logo_src = Path(__file__).resolve().parent.parent / "data" / "logo"
            if _logo_src.exists():
                n_logo = zip_dir(zf, _logo_src, "7tan-editor/data/logo",
                                 exclude_files=EXCLUDE_FILE_PATTERNS | _SECURITY_SRC_EXCLUDE)
                print(f"  [OK] 7tan-editor/data/logo/  {n_logo} 文件")
                n_total += n_logo

            # 2.1.2 exe 同级 tools/web_autostart.py（网页版一键启动依赖）
            # main.py 的 --web-autostart 从 PROJECT_ROOT（打包后 = exe 所在目录）/tools/
            # web_autostart.py 加载自启工具；PyInstaller 只把该文件打进了 _internal/tools/，
            # exe 同级缺失时「启动网页版服务(Windows).bat」会静默失败
            # （窗口程序 console=False，报错无法显示，只能看到"启动失败"）。
            _autostart_src = Path(__file__).resolve().parent.parent / "tools" / "web_autostart.py"
            if _autostart_src.exists():
                _safe_write(zf, _autostart_src, "7tan-editor/tools/web_autostart.py")
                print(f"  [OK] 7tan-editor/tools/web_autostart.py ({_autostart_src.stat().st_size:,}B)")
                n_total += 1
            else:
                print("  [WARN] tools/web_autostart.py 缺失，网页版一键启动将无法工作")
