# -*- coding: utf-8 -*-
"""最大化微信窗口 + 确认主界面 + 执行搜索"""
import ctypes, sys, io, time
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
import ctypes.wintypes as wt
import pyautogui, pyperclip
from PIL import Image
import mss

user32 = ctypes.windll.user32
WX_HWND = 526136

# 最大化
user32.ShowWindow(WX_HWND, 3)  # SW_MAXIMIZE
time.sleep(1.0)
user32.SetForegroundWindow(WX_HWND)
time.sleep(0.5)

rect = wt.RECT()
user32.GetWindowRect(WX_HWND, ctypes.byref(rect))
print(f"微信窗口: ({rect.left},{rect.top})-({rect.right},{rect.bottom})")
left, top = rect.left, rect.top
w, h = rect.right-rect.left, rect.bottom-rect.top

def grab(box=None):
    box = box or (left, top, w, h)
    with mss.mss() as sct:
        shot = sct.grab({"left": box[0], "top": box[1], "width": box[2], "height": box[3]})
    return Image.frombytes("RGB", shot.size, shot.rgb)

def ocr(img, scale=2, region=None):
    if region:
        img = img.crop(region)
    img2 = img.resize((img.width*scale, img.height*scale), Image.LANCZOS)
    buf = io.BytesIO(); img2.save(buf, format="PNG"); data = buf.getvalue()
    from winrt.windows.media.ocr import OcrEngine
    from winrt.windows.globalization import Language
    from winrt.windows.graphics.imaging import BitmapDecoder
    from winrt.windows.storage.streams import InMemoryRandomAccessStream, DataWriter
    engine = OcrEngine.try_create_from_language(Language("zh-CN"))
    stream = InMemoryRandomAccessStream(); w = DataWriter(stream)
    w.write_bytes(data)
    if hasattr(w, "store"): w.store()
    else: w.store_async().get()
    stream.seek(0)
    decoder = BitmapDecoder.create_async(stream).get()
    bmp = decoder.get_software_bitmap_async().get()
    res = engine.recognize_async(bmp).get()
    return [(l.text, l.words[0].bounding_rect if l.words else None) for l in res.lines if l.text.strip()]

img = grab()
img.save(r"D:\7tan\7tanAI\data\screenshots\wx_maximized.png")
print("\n=== 最大化后界面 OCR ===")
for t, r in ocr(img, 2)[:30]:
    pos = f"({int(r.x/2)},{int(r.y/2)})" if r else ""
    print(f"  {t} {pos}")

# 如果界面正常（有聊天列表），继续搜索
texts = [t for t, _ in ocr(img, 2)]
if len(texts) > 5:
    # 搜索框位置：左侧栏顶部（相对窗口，现在窗口是最大化的）
    # 搜索框约在相对 (150, 55) → 绝对 (left+150, top+55)
    sx, sy = left + 150, top + 55
    print(f"\n[CLICK] 搜索框 ({sx},{sy})")
    pyautogui.click(sx, sy)
    time.sleep(0.8)
    pyperclip.copy("游戏推荐")
    time.sleep(0.2)
    pyautogui.hotkey("ctrl", "a")
    time.sleep(0.2)
    pyautogui.hotkey("ctrl", "v")
    time.sleep(0.5)
    print("[TYPE] 游戏推荐")
    pyautogui.press("enter")
    time.sleep(2.5)
    print("[ENTER] 回车")
    img2 = grab()
    img2.save(r"D:\7tan\7tanAI\data\screenshots\search_result_v2.png")
    print("\n=== 搜索结果页 OCR ===")
    for t, r in ocr(img2, 2)[:40]:
        pos = f"({int(r.x/2)},{int(r.y/2)})" if r else ""
        print(f"  {t} {pos}")
else:
    print("\n界面内容仍不足，可能需要点击进入主界面")
