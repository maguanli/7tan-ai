...
ensure_front()
# 点击「文章」tab：窗口内 (828,165)
pyautogui.click(x0 + 828, y0 + 165)
time.sleep(3.0)

img = snap()
img.save(r'D:\7tan\7tanAI\tools\ocr_work\article_tab.png')
words, text = ocr_all(img)
with open(r'D:\7tan\7tanAI\tools\ocr_work\article_tab.txt', 'w', encoding='utf-8') as f:
    for wx, wy, ww, wh, wt in words:
        f.write(f"({wx},{wy},{ww},{wh}) | {wt}\n")
    f.write("\nFULL:\n" + text)
print("LINES:", len(words))
for wx, wy, ww, wh, wt in words[:60]:
