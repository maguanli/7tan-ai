...
def main():
    ...
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 300, 'top': mon['top'] + 420, 'width': 640, 'height': 160}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/digest_after.png')
    text = asyncio.run(recognize('data/screenshots/digest_after.png'))
    print('=====DIGEST ZONE=====')
    print(text)
    print('=====END=====')
