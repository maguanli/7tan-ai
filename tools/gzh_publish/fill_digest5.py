# -*- coding: utf-8 -*-
"""点击摘要框(680,564)粘贴摘要，OCR验证0/120变化"""
...
def main():
    ...
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 400, 'top': mon['top'] + 510, 'width': 560, 'height': 120}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/digest_v5.png')
    words = asyncio.run(ocr_words('data/screenshots/digest_v5.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+400},{y+510})')
