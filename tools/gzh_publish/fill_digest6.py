...
def main():
    ...
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 400, 'top': mon['top'] + 480, 'width': 560, 'height': 140}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/digest_v6.png')
    words = asyncio.run(ocr_words('data/screenshots/digest_v6.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+400},{y+480})')
