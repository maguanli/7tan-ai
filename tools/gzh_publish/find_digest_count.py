...
KEY = ['120', '/120', '0', '摘要', '封面', '拖拽', '原创', '留言', '合集', '原文', '创作', '广告', '平台', '推荐', '草稿', '发表', '赞赏', '开启', '未添加', '已开']

def main():
    with mss.mss() as sct:
        img = sct.grab(sct.monitors[1])
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/full_words.png')
    words = asyncio.run(ocr_words('data/screenshots/full_words.png'))
    for text, x, y, w, h in words:
        t = text.strip()
        if t in KEY or t == '120':
            print(f'{t!r} @ ({x},{y})')
