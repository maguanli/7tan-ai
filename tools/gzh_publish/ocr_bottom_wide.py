# -*- coding: utf-8 -*-
"""OCR全宽度底部 (x300-1920, y770-950) 找按钮"""
...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 300, 'top': mon['top'] + 770, 'width': 1620, 'height': 180}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/bottom_wide.png')
    words = asyncio.run(ocr_words('data/screenshots/bottom_wide.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+300},{y+770})')
