...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 300, 'top': mon['top'] + 350, 'width': 760, 'height': 500}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/original_dialog.png')
    words = asyncio.run(ocr_words('data/screenshots/original_dialog.png'))
