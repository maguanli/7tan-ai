...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 300, 'top': mon['top'] + 840, 'width': 760, 'height': 160}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/dialog_bottom.png')
    words = asyncio.run(ocr_words('data/screenshots/dialog_bottom.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+300},{y+840})')
