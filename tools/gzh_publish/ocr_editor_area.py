# -*- coding: utf-8 -*-
"""OCR公众号编辑器区域（屏幕中间 x 180-900, y 120-700）"""
...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 180, 'top': mon['top'] + 120, 'width': 720, 'height': 580}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/editor_area.png')
        print('saved', img.size)
    text = asyncio.run(recognize('data/screenshots/editor_area.png'))
    print('=====OCR EDITOR AREA=====')
    print(text)
    print('=====END=====')
