# -*- coding: utf-8 -*-
"""OCR编辑器右半部分 (x560-900, y250-560) 找设置栏"""
...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 560, 'top': mon['top'] + 250, 'width': 340, 'height': 310}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/editor_right_col.png')
        print('saved', img.size)
    text = asyncio.run(recognize('data/screenshots/editor_right_col.png'))
    print('=====RIGHT COL=====')
    print(text)
    print('=====END=====')
