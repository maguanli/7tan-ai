...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 300, 'top': mon['top'] + 430, 'width': 600, 'height': 190}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/original_row.png')
    words = asyncio.run(ocr_words('data/screenshots/original_row.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+300},{y+430})')
