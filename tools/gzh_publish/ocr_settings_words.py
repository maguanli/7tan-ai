...
def main():
    with mss.mss() as sct:
        mon = sct.monitors[1]
        region = {'left': mon['left'] + 180, 'top': mon['top'] + 400, 'width': 720, 'height': 680}
        img = sct.grab(region)
        img = Image.frombytes('RGB', img.size, img.rgb)
        img.save('data/screenshots/editor_settings_words.png')
    words = asyncio.run(ocr_words('data/screenshots/editor_settings_words.png'))
    for text, x, y, w, h in words:
        print(f'{text!r} @ screen({x+180},{y+400})')
