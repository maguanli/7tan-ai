# -*- coding: utf-8 -*-
"""set_cover2.py — 关闭误开弹窗, 正确定位并设置封面"""
import asyncio
import difflib
import io
import subprocess
...
def main():
    ...
    words = ocr_words(img)
    # 2. 找封面区文字「拖拽」或「封面」
    hit = find_key(words, "拖拽")
    if not hit:
        hit = find_key(words, "封面")
    if not hit:
        # 兜底: 找「52/120」, 封面在其上方 90px
        hit2 = find_key(words, "120")
        if hit2:
            hit = {"x": 700, "y": hit2["y"] - 90, "line": f"fallback from 52/120 @ {hit2['y']}"}
    if not hit:
        print("COVER_AREA_NOT_FOUND")
        return
    print(f"CLICK_COVER at ({hit['x']},{hit['y']}) line={hit['line']}")
    click(hit["x"], hit["y"])
    time.sleep(2.5)
    ...
    for attempt in range(3):
        ...
        if up:
            print(f"CLICK_UPLOAD at ({up['x']},{up['y']})")
            click(up["x"], up["y"])
            time.sleep(2.0)
            type_path(COVER)
            break
        print(f"upload btn not found (attempt {attempt}), esc+retry")
        pyautogui.press("esc")
        time.sleep(1.0)
    else:
