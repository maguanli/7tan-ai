# -*- coding: utf-8 -*-
"""滚回顶部 → 点击10万+爆款文章 → 读取全文"""
import sys, io, time
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
sys.path.insert(0, r'D:\7tan\7tanAI\tools')
from PIL import Image
import win32gui, win32con, mss
import pyautogui
import wechat_scan as ws

pyautogui.FAILSAFE = False
pyautogui.PAUSE = 0.3

WX_HWND = 657102
rect = win32gui.GetWindowRect(WX_HWND)
x0, y0, x1, y1 = rect
w, h = x1-x0, y1-y0

def ensure_front():
    if win32gui.GetForegroundWindow() != WX_HWND:
        win32gui.ShowWindow(WX_HWND, win32con.SW_SHOW)
        win32gui.SetForegroundWindow(WX_HWND)
        time.sleep(1.0)

def snap():
    with mss.MSS() as sct:
        shot = sct.grab({'left': x0, 'top': y0, 'width': w, 'height': h})
    return Image.frombytes('RGB', shot.size, shot.rgb)

def ocr_all(img, scale=2.0):
    big = img.resize((int(img.width*scale), int(img.height*scale)), Image.LANCZOS)
    words, text = ws.ocr_words(big, scale=scale)
    return words, text.replace(' ', '')

ensure_front()
# 滚回顶部：鼠标移到结果区，向上滚动多次
pyautogui.moveTo(x0 + 850, y0 + 400)
time.sleep(0.5)
for _ in range(10):
    pyautogui.scroll(500)
    time.sleep(0.4)
time.sleep(2.0)

img = snap()
words, text = ocr_all(img)
print("TOP SCREEN:", text[:200])

# 找「6款超魔性的微信小游戏」标题行
target = None
for wx, wy, ww, wh, wt in words:
    if '超魔性' in wt.replace(' ', '') or ('6款' in wt.replace(' ', '') and '小游戏' in wt.replace(' ', '')):
        target = (wx + ww//2, wy + wh//2)
        print(f"找到文章标题: ({wx},{wy}) -> 绝对({x0+wx+ww//2},{y0+wy+wh//2}) | {wt}")
        break
if target:
    pyautogui.click(x0 + target[0], y0 + target[1])
    time.sleep(4.0)
    img2 = snap()
    img2.save(r'D:\7tan\7tanAI\tools\ocr_work\article_open.png')
    words2, text2 = ocr_all(img2)
    with open(r'D:\7tan\7tanAI\tools\ocr_work\article_open.txt', 'w', encoding='utf-8') as f:
        for wx, wy, ww, wh, wt in words2:
            f.write(f"({wx},{wy},{ww},{wh}) | {wt}\n")
        f.write("\nFULL:\n" + text2)
    print("=== 打开文章后 ===")
    print(text2[:600])
else:
    print("未找到文章标题！")
