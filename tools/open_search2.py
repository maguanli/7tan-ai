# -*- coding: utf-8 -*-
"""打开搜索页 (80,179) → 修正坐标 OCR（放大2x后 scale=2.0 还原）"""
import sys, io, time
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
sys.path.insert(0, r'D:\7tan\7tanAI\tools')
from PIL import Image
import win32gui, win32con, mss
import pyautogui
import wechat_scan as ws

pyautogui.FAILSAFE = False
pyautogui.PAUSE = 0.3

WX_HWND = 657102
rect = win32gui.GetWindowRect(WX_HWND)
x0, y0, x1, y1 = rect
w, h = x1-x0, y1-y0

def ensure_front():
    if win32gui.GetForegroundWindow() != WX_HWND:
        win32gui.ShowWindow(WX_HWND, win32con.SW_SHOW)
        win32gui.SetForegroundWindow(WX_HWND)
        time.sleep(1.0)

def snap():
    with mss.MSS() as sct:
        shot = sct.grab({'left': x0, 'top': y0, 'width': w, 'height': h})
    return Image.frombytes('RGB', shot.size, shot.rgb)

ensure_front()
# 打开搜索页（左侧窄栏搜索图标）
pyautogui.click(x0 + 80, y0 + 179)
time.sleep(2.0)

img = snap()
img.save(r'D:\7tan\7tanAI\tools\ocr_work\sp2.png')
big = img.resize((img.width*2, img.height*2), Image.LANCZOS)
words, text = ws.ocr_words(big, scale=2.0)  # 坐标÷2 还原到原图

with open(r'D:\7tan\7tanAI\tools\ocr_work\sp2.txt', 'w', encoding='utf-8') as f:
    f.write("=== WORDS (窗口内坐标,已还原) ===\n")
    for wx, wy, ww, wh, wt in words:
        f.write(f"({wx},{wy},{ww},{wh}) | {wt}\n")
    f.write("\n=== FULL ===\n" + text.replace(' ', ''))
print("LINES:", len(words))
for wx, wy, ww, wh, wt in words[:50]:
    print(f"  ({wx},{wy},{ww},{wh}) | {wt}")
