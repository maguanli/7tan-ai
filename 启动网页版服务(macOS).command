...
# ============================================================
# 7Tan WEB 版启动器（macOS）
# 解压后双击本文件 → 自动安装依赖 → 启动 9900 WEB 服务 → 打开浏览器
# 首次双击若提示权限，请右键 → 打开 即可
# ============================================================
cd "$(dirname "$0")" || exit 1
mkdir -p logs
...
echo "=============================================="
echo "  7Tan WEB 版启动器 (macOS)"
echo "=============================================="

# [1/4] 检测系统
UNAME=$(uname -s)
if [ "$UNAME" != "Darwin" ]; then
  echo "❌ 本脚本仅支持 macOS；Linux 请执行「Linux-启动网页版服务.sh」"
  read -r -p "按回车退出..." _
  exit 1
fi
echo "[1/4] ✅ 检测系统: macOS ($(uname -m))"

# [2/4] 检测 Python3
PY=python3
if ! command -v "$PY" >/dev/null 2>&1; then
  echo "❌ 未检测到 Python3，请先安装："
  echo "   → 打开 https://www.python.org/downloads/ 下载 macOS 安装包并安装"
